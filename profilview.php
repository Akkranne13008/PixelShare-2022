...
$requeteliketoto = $bdd->prepare('SELECT SUM(Note) AS total_note FROM Dessin WHERE Auteur = "' . $_GET['profil'] .'"');
$requeteliketoto->execute();
$reponseliketoto = $requeteliketoto->fetch(PDO::FETCH_ASSOC);
...
<li>Pseudo : <span><?php print_r($_GET['profil']) ?></span></li>
<li>Total Like : <span><?php print_r($reponseliketoto['total_note']) ?></span></li>
<li>Follow : <span><?php print_r($reponseliketoto['total_note']) ?></span></li>
<li>Follow : <span><?php print_r($reponseliketoto['total_note']) ?></span></li>
<?php
echo'<form action="like.php?like='. $reponseimage['ID_Dessin']. '&cuser='.$_SESSION['id'].'" method="post">	
            <button class="btn-floating halfway-fab waves-effect waves-light grey"><i>❤</button></a>
            </form>
            ';
?>
<?php 
...
$requete = 'SELECT * FROM Dessin WHERE Auteur="' . $_GET['profil'] .'" ORDER BY Note DESC ';
